...
class OpenStackInventory(object):
    ...
    def get_host_groups_from_cloud(self):
        ...
        lb_vips = collections.defaultdict(dict)
        ...
        for cloud in self.clouds:
            ...
            ## Add lb_vips inventory 
            neutron = cloud.neutron_client
            vips = neutron.list_vips()
            floating_ips = neutron.list_floatingips()
            for vip in vips['vips']:
                pattern = re.compile("^(.*)-lb_vip-(.*)")
                lbvipgroup = pattern.sub(r"lb_vip-\2", vip['name'])
                groups[lbvipgroup].append(vip['name'])
                hostvars[vip['name']] = vip
                hostvars[vip['name']]['region_name'] = cloud.region_name
                for floatingip in floating_ips['floatingips']:
                    if vip['port_id'] == floatingip['port_id']:
                        hostvars[vip['name']]['floating_ip_address'] = floatingip['floating_ip_address']
        ...
        # floating_ips = neutron.list_floatingips()
        # for vip in vips['vips']:
        #     groups[group].append(vip['name'])
        #     groups['_meta']['hostvars'][vip['name']] = vip
        #     for floatingip in floating_ips['floatingips']:
        #         if vip['port_id'] == floatingip['port_id']:
        #             groups['_meta']['hostvars'][vip['name']]['floating_ip_address'] = floatingip['floating_ip_address']
